...
#parametres de base
{
  ...
  d_eta=1;eta_max= 240;Veta=seq(0,eta_max,by=d_eta);Neta=length(Veta);
...
VectProbaDeathThreshold=c(0.5,0.4)#,0.6,0.75
...
for (PropIVM in VectPropIVM) {
  ...
  for (ProbaDeathThreshold in VectProbaDeathThreshold) {
    ...
    pdf(FigName, width = 8, height = 6.4)  
    ...
    for (nc in 2:3) {
      ...
      if(is.element(GrNumber,c(1,2))){
        mtext(bquote("mdc-STM-001-0.6 when " ~ n[c] == .(nc)),
    ...
    #Bohemia
    {
      ...
      for (Ncp in 1:length(VectCoefDelay)) {
        ...
        lines(time, ModelBohemia[[Ncp]]$PropIhTot, lwd = LC, cex.lab = 1.2, 
              lty = LineVect[Ncp], col = ColVect[Ncp])
      }
      
      mtext("BOHEMIA", side = 3, adj = 0.5, cex = 0.8, line = 1, font = 1)
      ...
      # 
      # # legend("bottom",  
      # #        legend = c("mdc-STM-001-0.6","RIMDAMAL II" ), inset = c(0,0.055),
...
#Figure: 4 & 5 Scenarios_Formulations Efficacy # 75%, 60%, 50%, 40%
###########################
CoefDelay=0.75
...
VectProbaDeathThreshold=c(0.75,0.6,0.5,0.4)
...
for (PropIVM in VectPropIVM) {
  ...
  FigName = paste0("Scenario_Eff_Ih_AgeDependece",IVM_age_dependancy,"PropIVM_",
                   gsub("\\.","_",PropIVM) , ".pdf")
  pdf(FigName, width = 12, height = 11.2)  
  par(oma = c(5.5, 1, 2, .1), mar = c(3, 3.5, 2, .1))
  ...
  for (ProbaDeathThreshold in VectProbaDeathThreshold) {
    ...
    for (nc in 1:3) {
      ...
      if(is.element(GrNumber,c(1,4,7,10))){
        mtext("Clinical cases (%)", side = 2, adj = 0.5, cex = .85, line = 2, font = .8) 
        mtext(bquote(Eff == .(ProbaDeathThreshold * 100) * "%"), 
              side = 2, adj = 0.5, cex = 0.9, line = 3, font = 1)
      }
      
      if(is.element(GrNumber,c(10,11,12))){
        mtext("Time (Days)", side = 1, adj = 0.5, cex = .9, line = 2, font = .8) 
      }
    }
  ...
  #plot legend
  LEGEND=c("mdc-STM-001−0.6", "mdc-STM-001−1.0", "BOHEMIA", "RIMDAMAL II")
  reset()
  legend("bottom", legend=LEGEND, 
  ...
  for (k in 1:length(VectProbaDeathThreshold)) {
    for (i in 1:length(VectNumber_of_cycle)) {
      ...
      plot(-1, 1, type = "l", xlab = "", xlim = c(min(VectPropIVM), 1), ylab = "",
           ylim = c(0, 0.77), cex.lab = 1, yaxt = "n", xaxt = "n")
  ...
  #plot Proportion of dead mosquitoes: LongLasting et Orale
  {
    Plot_Prop_dead_mosquitoesLongLasting=ggplot() +
      ...
      scale_color_manual(values = c("F31-0.6" = "#ff3355", "Lam-1.0" = "#037153"),
                         labels = c("mdc-STM-001-0.6", "mdc-STM-001-1.0")) +
