...
setwd("C:/Users/franc/Desktop/PowerPoint PhD/R studio/Project IVM3")
...
#parametres de base
{
  ...
  mu_m = 0.13  
  t_begin_sum=250 # par defaut, pas utile ici
  IVM_Pregnancy=0
  EIP=8
  # theta=0.5
  Dur_cycle=7 
...
IVM_Pregnancy=0
Prev_mos10=1 #Prevalence des moustiques infectieux 10%
...
#GainPrev_matrix_oral_Seas[1,,] pour BOHEMIA, GainPrev_matrix_oral_Seas[2,,] pour RIMDAMAL
#GainPrev_matrix_oral_Seas[3,,] pour Kamau RIMDAMAL
#dans le meme sens pour GainPrev_array_oral_Per
...
# GainPrev_array_LAIF_Seas[1,,,] pour "mdc-STM-001_0.6-vk5", GainPrev_array_LAIF_Seas[2,,,] pour "mdc-STM-001_1.0"
# GainPrev_array_LAIF_Seas[3,,,] pour "mdc-STM-001_0.6-kis"
#dans le meme sens pour GainPrev_array_LAIF_Per
...
{ 
  ...
  for (nc in VectNumber_of_cycle) {
    ...
    # Positions sur l'axe x (même logique que l'original)
    n_strategies = 6
    ...
    box()
    ticks = pretty(c(ylim[1], ylim[2]), n=4)
    ticks = sort(unique(c(ticks, 0)))
    axis(2, at=ticks)
    
    if(is.element(GrNumber,c(1,3))){
      mtext(expression("Performance" ~ "(" * "seasonal – perennial" * ")" ~ "(%)"), 
...
# Fonction recharger et reploter
